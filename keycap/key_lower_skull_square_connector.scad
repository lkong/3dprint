...
module connector() 
{
    ...
    {
        cube([c_vert+c_corr,c_vert_dia+c_corr,c_depth+.1], center=true );
        cube([c_horiz_dia+c_corr,c_horiz+c_corr,c_depth+.1], center=true );
    }
}
...
scale(key_scale) difference()
{
    ...
    { 
        key();
        translate([-c_length/2,-c_width/2,0]) cube([c_length,c_width, c_depth]);
    }
